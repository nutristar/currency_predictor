...
t_n = time()
if not os.path.exists('datas80'):
    os.makedirs('datas80')
if not os.path.exists('datas20'):
    os.makedirs('datas20')

def divider80_20(each):
    try:

        df = pd.read_csv(f"datas2/{each}.csv")
        ...
        df_train.to_csv(f"datas80/{each}80.csv", index=False)
        df_test.to_csv(f"datas20/{each}20.csv", index=False)
...
with concurrent.futures.ThreadPoolExecutor() as executor:
    ...
    executor.map(divider80_20, tickets_list)
